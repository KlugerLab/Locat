    ...
    def gmm_scan_new(
        ...
        for i_gene in tqdm(
            ...
            try:
                ...
                # match notebook behavior: clip sens then use (1 - sens) (sens_power==1.0)
                sens = np.clip(sens_score, 1e-6, 1.0)
                p_sens = _safe_p(1.0 - sens)
                # use notebook weights (w_size=0.05, w_sens=0.12)
                p_final = 1.0 - (1.0 - p_cauchy) * (1.0 - 0.05 * p_size) * (1.0 - 0.12 * p_sens)
